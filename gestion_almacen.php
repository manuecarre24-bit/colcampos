...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .header {
            background: <?php echo $color; ?>; color: #fff; padding: 14px 22px;
        ...
        th {
            ...
            padding: 12px 8px; border: 1px solid rgba(255,255,255,0.2);
        ...
        .btn-agregar:hover { filter: brightness(0.93); transform: scale(1.02); }
        ...
        .btn-save {
            background: <?php echo $color; ?>; color: #fff; padding: 13px 30px;
...
<div class="info-badge">
    ...
    &nbsp;|&nbsp; <span style="color:#e65100;">⚠ Naranja = stock bajo</span>
    <?php endif; ?>
</div>
...
<form action="guardar_almacen.php" method="POST">
    ...
    <table id="tablaAlmacen">
        ...
        <tbody id="cuerpo-almacen">
        <?php for ($i = 0; $i < $total_filas; $i++):
            $r     = $registros[$i] ?? null;
            $tiene = $r !== null;
            $bajo  = $tiene && floatval($r['cantidad_actual']) <= floatval($r['stock_minimo']) && floatval($r['stock_minimo']) > 0;
            $clase = $bajo ? 'stock-bajo' : ($tiene ? 'tiene-datos' : 'fila-nueva');
        ?>
        <tr class="<?php echo $clase; ?>">
            <td><div class="num-cell"><?php echo $i+1; ?></div></td>
            <td>
                <select name="categoria[]" class="inp <?php echo !$tiene?'inp-nueva':''; ?>">
                    <?php foreach (['Alimento','Medicamento','Herramienta','Insumo General'] as $cat):
                        $sel = ($tiene && $r['categoria'] == $cat) ? 'selected' : ''; ?>
                    <option value="<?php echo $cat; ?>" <?php echo $sel; ?>><?php echo $cat; ?></option>
                    <?php endforeach; ?>
                </select>
            </td>
            <td><input type="text" name="nombre_articulo[]" class="inp <?php echo !$tiene?'inp-nueva':''; ?>"
                value="<?php echo $tiene ? htmlspecialchars($r['nombre_articulo']) : ''; ?>"
                placeholder="Ej: Purina postura..."></td>
            <td><input type="number" name="cantidad_actual[]" class="inp <?php echo !$tiene?'inp-nueva':''; ?>"
                step="0.01" min="0" value="<?php echo $tiene ? $r['cantidad_actual'] : '0'; ?>"></td>
            <td><input type="text" name="unidad_medida[]" class="inp <?php echo !$tiene?'inp-nueva':''; ?>"
                value="<?php echo $tiene ? htmlspecialchars($r['unidad_medida']) : ''; ?>"
                placeholder="Kg / L / Und"></td>
            <td><input type="number" name="stock_minimo[]" class="inp <?php echo !$tiene?'inp-nueva':''; ?>"
                min="0" value="<?php echo $tiene ? $r['stock_minimo'] : '0'; ?>"></td>
        </tr>
        <?php endfor; ?>
        </tbody>
    </table>
    ...
    <div class="footer-form">
        <p>✅ Verde = guardado &nbsp; ⚠️ Naranja = stock bajo &nbsp;|&nbsp; Agrega los artículos que necesites</p>
...
<script>
var contadorAlmacen = <?php echo $total_filas; ?>;

function agregarFila() {
    var tbody = document.getElementById('cuerpo-almacen');
    var i = contadorAlmacen;
    var tr = document.createElement('tr');
    tr.className = 'fila-nueva';
    tr.innerHTML =
        '<td><div class="num-cell">' + (i + 1) + '</div></td>' +
        '<td><select name="categoria[]" class="inp inp-nueva">' +
            '<option value="Alimento">Alimento</option>' +
            '<option value="Medicamento">Medicamento</option>' +
            '<option value="Herramienta">Herramienta</option>' +
            '<option value="Insumo General">Insumo General</option>' +
        '</select></td>' +
        '<td><input type="text" name="nombre_articulo[]" class="inp inp-nueva" placeholder="Ej: Purina postura..."></td>' +
        '<td><input type="number" name="cantidad_actual[]" class="inp inp-nueva" step="0.01" min="0" value="0"></td>' +
        '<td><input type="text" name="unidad_medida[]" class="inp inp-nueva" placeholder="Kg / L / Und"></td>' +
        '<td><input type="number" name="stock_minimo[]" class="inp inp-nueva" min="0" value="0"></td>';
    tbody.appendChild(tr);
    contadorAlmacen++;
    tr.querySelector('input[name="nombre_articulo[]"]').focus();
}
</script>
</body>
</html>
